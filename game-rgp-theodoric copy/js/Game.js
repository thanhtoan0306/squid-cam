...
console.log('User photo profile URL:', userPhotoProfile);
...
Theodoric.Game.prototype = {
  ...
  showLabels: function () {
    ...
    this.healthLabel = this.game.add.text(
      225,
      this.game.height - 50,
      text,
      style
    ...
    this.goldLabel = this.game.add.text(
      this.game.width - 75,
      this.game.height - 25,
      text,
      style
    ...
    style = { font: '10px Arial', fill: '#fff', align: 'center' };
    this.spellLabel = this.game.add.text(
      230,
      this.game.height - 25,
      text,
      style
  ...
  attack: function (attacker, attacks) {
    if (
      attacker.alive &&
      this.game.time.now > attacks.next &&
      attacks.countDead() > 0
    ) {
      ...
      if (attacks.name === 'sword') {
        ...
        a.rotation = this.game.physics.arcade.moveToObject(
          a,
          this.player,
          attacks.range
        );
  ...
  hit: function (target, attacker) {
    if (this.game.time.now > target.invincibilityTime) {
      target.invincibilityTime =
        this.game.time.now + target.invincibilityFrames;
  ...
  collect: function (player, collectable) {
    if (!collectable.collected) {
      ...
      if (collectable.name === 'gold') {
        ...
        this.notification =
          'You consume a potion, increasing your vitality by ' +
          collectable.value +
          '!';
        this.potionSound.play();
        collectable.destroy();
        ...
        this.notification =
          'You consume a potion, increasing your strength by ' +
          collectable.value +
          '!';
        this.potionSound.play();
        collectable.destroy();
        ...
        this.notification =
          'You consume a potion, increasing your speed by  ' +
          collectable.value +
          '!';
        this.potionSound.play();
        collectable.destroy();
  ...
  generateEnemy: function () {
    ...
    do {
      ...
      Phaser.Math.distance(this.player.x, this.player.y, enemy.x, enemy.y) <=
      400
    );

    var rnd = Math.random();
    if (rnd >= 0 && rnd < 0.3) enemy = this.generateSkeleton(enemy);
    else if (rnd >= 0.3 && rnd < 0.4) enemy = this.generateSlime(enemy);
    else if (rnd >= 0.4 && rnd < 0.6) enemy = this.generateBat(enemy);
    else if (rnd >= 0.6 && rnd < 0.7) enemy = this.generateGhost(enemy);
    else if (rnd >= 0.7 && rnd < 1) enemy = this.generateSpider(enemy);
  ...
  generateSlime: function (enemy) {
    ...
    // --- Add this block to show the user photo above the slime ---
    // Only add for Slime, not other enemies
    const photoKey = 'slime_user_photo';
    if (!this.game.cache.checkImageKey(photoKey)) {
      this.game.load.image(photoKey, userPhotoProfile);
      this.game.load.once('complete', () => {
        const photo = this.game.add.image(enemy.x, enemy.y - 40, photoKey);
  ...
  generateChest: function (location) {
    var collectable = this.collectables.create(
      location.x,
      location.y,
      'things'
    );
  ...
  enemyMovementHandler: function (enemy) {
    ...
    if (
      ...
      enemy.body.velocity.x <= -Math.abs(enemy.body.velocity.y)
    ) {
      ...
      enemy.body.velocity.y <= -Math.abs(enemy.body.velocity.x)
    ) {
